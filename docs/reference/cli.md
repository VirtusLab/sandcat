...
Options:
- `--agent` - Agent type: `claude`, `cursor`, `codex`, `copilot` (skips prompt)
...
The published image (`ghcr.io/virtuslab/sandcat-mitmproxy-pass`) is built in CI, but you can build it locally. The pinned `pass-cli` version and per-arch checksums live in a single source of truth, [`images/mitmproxy-pass/pass-cli.env`](https://github.com/VirtusLab/sandcat/blob/master/images/mitmproxy-pass/pass-cli.env), and must be passed in as build args (the Dockerfile has no defaults on purpose):
