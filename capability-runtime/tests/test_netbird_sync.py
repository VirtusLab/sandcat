...
def test_rest_enable_binding_patches_existing_route(monkeypatch):
    captured: dict = {}
    ...
    assert json.loads(captured["body"]) == {"enabled": True}
    assert updated.route_id == "route-1"


def test_rest_disable_binding_patches_route_disabled(monkeypatch):
    captured: dict = {}
    ...
    assert captured["method"] == "PATCH"
    assert captured["url"] == "https://api.netbird.io/api/routes/route-1"
    assert json.loads(captured["body"]) == {"enabled": False}
