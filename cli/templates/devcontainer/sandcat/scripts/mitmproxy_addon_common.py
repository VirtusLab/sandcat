...
class SandcatAddon:
    ...
    def running(self):
        """Called when mitmweb is running — CA cert is fully generated by
        this point (mitmproxy's certs addon lazily creates it during TLS
        engine startup, which happens after our addon's load()). Seeding
        from here guarantees the cert is present on the public volume
        before the mitmproxy healthcheck passes and the agent starts."""
    ...
    def _seed_public_ca_cert(self):
        ...
        TLS setup; called from running() where CA is guaranteed present,
        but we retain a retry loop as a defensive belt-and-braces for
        slow CI environments. See issue #25.
        """
        ...
        for _ in range(30):
            ...
            time.sleep(1)
        ctx.log.warn(
            f"CA cert not present at {src} after 30s — agent may see no cert"
        )
