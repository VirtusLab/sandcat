...
@test "customize_devbox adds stack packages to devbox.stack.json" {
	...
	run jq -e '.packages | any(. == "javaPackages.compiler.openjdk21@latest")' "$DEVCONTAINER_DIR/devbox.stack.json"
	assert_success
}
...
@test "devcontainer command flows --stacks into devbox.stack.json" {
	...
	run jq -r '.packages | length' "$PROJECT_DIR/.devcontainer/devbox.stack.json"
	assert_output "11"
	run jq -e '.packages | any(. == "javaPackages.compiler.openjdk21@latest")' "$PROJECT_DIR/.devcontainer/devbox.stack.json"
	assert_success
}
