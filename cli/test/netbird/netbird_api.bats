...
@test "netbird_status calls GET /api/peers" {
    stub curl \
        "-sS -f -X GET -H 'Authorization: Token test-token' -H 'Accept: application/json' -H 'Content-Type: application/json' https://api.netbird.io/api/peers : echo '[{\"id\":\"peer1\",\"connected\":true}]'"
    run netbird_status
    assert_success
...
@test "netbird_route_add calls POST /api/routes with network and peer" {
    stub curl \
        "-sS -f -X POST -H 'Authorization: Token test-token' -H 'Accept: application/json' -H 'Content-Type: application/json' -d '{\"network\":\"10.8.0.0/24\",\"peer\":\"peer1\",\"enabled\":true}' https://api.netbird.io/api/routes : echo '{\"id\":\"route1\"}'"
...
@test "netbird_peer_remove calls DELETE /api/peers/:id" {
    stub curl \
        "-sS -f -X DELETE -H 'Authorization: Token test-token' -H 'Accept: application/json' -H 'Content-Type: application/json' https://api.netbird.io/api/peers/peer1 : :"
