...
@test "lifecycle replaces a same-name peer before up and labels it after start" {
	...
	assert_success
	replace_line=$(grep -n -m1 'netbird_replace_same_name_peer_if_needed || return 1' "$lifecycle" | cut -d: -f1)
	up_line=$(grep -n -m1 'netbird_up_enroll "$iface"' "$lifecycle" | cut -d: -f1)
...
@test "mitmproxy-init enrolls NetBird in the background after clearing health sentinels" {
	local init="$SCT_TEMPLATEDIR/devcontainer/sandcat/scripts/mitmproxy-init.sh"
	local main_body clear_line enroll_line start_line
	main_body=$(awk '/^main\(\)/,/^}/' "$init")
	printf '%s\n' "$main_body" | grep -F 'clear_mitmproxy_health_sentinels'
	printf '%s\n' "$main_body" | grep -F 'maybe_start_netbird_mesh &'
	printf '%s\n' "$main_body" | grep -F 'docker-entrypoint.sh'
	printf '%s\n' "$main_body" | grep -F 'trap netbird_mitmproxy_cleanup TERM INT'
	run grep -F 'INPUT -i "$iface" -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT' "$init"
	assert_success
	run grep -F 'INPUT -i "$iface" -j DROP' "$init"
	assert_success
	run grep -F '--dport 8081' "$init"
	assert_failure
	...
	start_line=$(printf '%s\n' "$main_body" | grep -n -m1 'docker-entrypoint.sh' | cut -d: -f1)
	[[ "$clear_line" -lt "$enroll_line" ]]
	[[ "$enroll_line" -lt "$start_line" ]]
	run grep -F 'exec docker-entrypoint.sh' "$init"
	assert_failure
}
...
@test "mitmproxy-init keeps NetBird WG port off mitmproxy WireGuard 51820" {
	...
	assert_success
	run grep -F -- '--wireguard-port "${NETBIRD_WG_PORT}"' "$lifecycle"
	assert_success
	run grep -F 'export NB_WIREGUARD_PORT=' "$lifecycle"
	assert_success
}
...
@test "compose run attach restart init export a filtered agent .sandcat mount" {
	local f
	for f in compose/compose run/run attach/attach restart/restart init/devcontainer; do
		run grep -F 'export_agent_sandcat_mount' "$SCT_ROOT/libexec/$f"
		assert_success
	done
}

@test "init --netbird looks up settings from the project path" {
	run grep -F 'SANDCAT_PROJECT_ROOT' "$SCT_ROOT/libexec/init/devcontainer"
	assert_success
}

@test "mitmproxy-init installs upstream CA bundles before starting mitmweb" {
	local init="$SCT_TEMPLATEDIR/devcontainer/sandcat/scripts/mitmproxy-init.sh"
	local main_body
	main_body=$(awk '/^main\(\)/,/^}/' "$init")
	printf '%s\n' "$main_body" | grep -F 'install_upstream_ca_bundles'
}
