...
@test "patch_dnsmasq_from_netbird_volume replaces stale host-record IPs" {
	...
	rm -f "$DNSMASQ_CONF.netbird-peers.stamp"
	patch_dnsmasq_from_netbird_volume "$DNSMASQ_CONF"
...
@test "patch_dnsmasq_from_netbird_volume skips merge when peers mtime is unchanged" {
	printf 'address=/test-proxy-peer.netbird.selfhosted/100.64.0.5\n' > "$NETBIRD_PEERS_CONF"
	patch_dnsmasq_from_netbird_volume "$DNSMASQ_CONF"
	local before
	before=$(cat "$DNSMASQ_CONF")
	...
	assert_equal "$(cat "$DNSMASQ_CONF")" "# existing config
server=1.1.1.1"
	[[ "$before" != "$(cat "$DNSMASQ_CONF")" ]]
}

@test "restart_dnsmasq does not start a second process if the old one is still listening" {
	rm -f "$DNSMASQ_PID_FILE"
	dnsmasq-ready() { return 0; }
	local started="$BATS_TEST_TMPDIR/dnsmasq.started"
	: >"$started"
	dnsmasq() { echo started >>"$started"; }

	run restart_dnsmasq "$DNSMASQ_CONF"
	assert_success
	assert_equal "$(cat "$started")" ""
}
